...
find ./proto -name '*.proto' -print0 | xargs -0 -I{} -P${CPUS} \
    docker run -v ${PWD}:/tmp/workspace -w /tmp/workspace namely/protoc-all:1.23_0 -f {} -o build/go -l go
...
docker run -it -v ${PWD}/build/go:/tmp/workspace -w /tmp/workspace unibeautify/goimports -w .
